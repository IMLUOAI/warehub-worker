...
async function handleRequest(request, env, ctx) {
    ...
    if (tenant.plan_expires_at && new Date(tenant.plan_expires_at) < new Date()) {
      return err('Subscription expired — please renew at app.warehub.com/billing', 402);
    }
    ...
    if (sessionMatch) {
      if (method === 'PATCH') return updateSession(request, tenant, sessionMatch[1], env);
    }
...
async function handleRegister(request, env) {
  ...
  await env.DB.prepare(
    `INSERT INTO users (id, tenant_id, clerk_user_id, email, name, role)
     VALUES (?, ?, ?, ?, ?, 'owner')`
  ).bind(userId, tenantId, clerkUserId, email, name).run();
...
async function getPackers(tenant, env) {
  ...
  for (const s of sessions) {
    ...
    sessionMap[s.packer_id].push({ clockIn: s.clock_in, clockOut: s.clock_out, id: s.id });
  }
...
async function syncPackers(request, tenant, env) {
  ...
  for (const p of list) {
    ...
    ).bind(p.id, tenant.id).first();

    if (existing) {
      await env.DB.prepare(
        `UPDATE packers SET name=?,color=?,pin=?,is_manager=?,online=?,
          daily_orders_date=?,daily_orders_completed=? WHERE id=? AND tenant_id=?`
      ).bind(p.name||'', p.color||'#4a9eff', p.pin||null,
        p.is_manager?1:0, p.online?1:0,
        p.dailyOrdersDate||'', p.dailyOrdersCompleted||0,
        p.id, tenant.id).run();
    } else {
      await env.DB.prepare(
        `INSERT INTO packers (id,tenant_id,name,color,pin,is_manager,online,
         ...
         VALUES (?,?,?,?,?,?,?,?,?)`
      ).bind(p.id, tenant.id, p.name||'', p.color||'#4a9eff', p.pin||null,
        p.is_manager?1:0, p.online?1:0,
        p.dailyOrdersDate||'', p.dailyOrdersCompleted||0).run();
    }
        ...
        if (!existSes) {
          await env.DB.prepare(
            `INSERT INTO packer_sessions (id,tenant_id,packer_id,clock_in,clock_out)
             VALUES (?,?,?,?,?)`
          ).bind(sessionId, tenant.id, p.id, clockIn, clockOut).run();
        } else if (clockOut) {
          await env.DB.prepare(
            `UPDATE packer_sessions SET clock_out=? WHERE id=?`
          ).bind(clockOut, sessionId).run();
        }
      }
    }
  }
...
async function createPacker(request, tenant, env) {
  ...
  await env.DB.prepare(
    `INSERT INTO packers (id, tenant_id, name, color, pin, is_manager)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).bind(id, tenant.id, b.name, b.color||'#4a9eff', b.pin||null, b.is_manager?1:0).run();
  return json({ id }, 201);
}

async function updatePacker(request, tenant, packerId, env) {
  const b = await request.json().catch(() => ({}));
  const allowed = ['name','color','pin','is_manager','online',
                   'daily_orders_date','daily_orders_completed'];
  const fields = [], values = [];
...
async function deletePacker(tenant, packerId, env) {
  await env.DB.prepare(
    `DELETE FROM packers WHERE id = ? AND tenant_id = ?`
  ).bind(packerId, tenant.id).run();
  return json({ ok: true });
}
...
async function getSessions(request, tenant, env) {
  ...
  const { results } = await env.DB.prepare(
    `SELECT * FROM packer_sessions
     ...
     ORDER BY clock_in DESC`
  ).bind(tenant.id, since).all();
  return json({ sessions: results });
}

async function createSession(request, tenant, env) {
  const b = await request.json().catch(() => ({}));
  if (!b.packer_id || !b.clock_in) return err('packer_id and clock_in required');
  const id = uuid();
  await env.DB.prepare(
    `INSERT INTO packer_sessions (id, tenant_id, packer_id, clock_in)
     VALUES (?, ?, ?, ?)`
  ).bind(id, tenant.id, b.packer_id, b.clock_in).run();
  return json({ id }, 201);
}
...
async function updateSession(request, tenant, sessionId, env) {
  ...
  ).bind(b.clock_out, sessionId, tenant.id).run();
  return json({ ok: true });
}
...
async function getOrders(tenant, env) {
  ...
  return json({ orders: orders.map(o => ({ ...o, skus: skuMap[o.id]||[] })) });
}

async function createOrders(request, tenant, env) {
  const b = await request.json().catch(() => ({}));
  if (!Array.isArray(b.orders) || !b.orders.length) return err('orders array required');
  let inserted = 0;
  for (const o of b.orders) {
    ...
    await env.DB.prepare(
      `INSERT INTO orders (id, tenant_id, tracking, carrier, shelf, status)
       VALUES (?, ?, ?, ?, ?, 'pending')`
    ).bind(id, tenant.id, o.tracking, o.carrier||'Unknown', o.shelf||null).run();
    if (o.skus && o.skus.length) {
      for (const s of o.skus) {
        await env.DB.prepare(
          `INSERT INTO order_skus (id, tenant_id, order_id, sku, qty)
           VALUES (?, ?, ?, ?, ?)`
        ).bind(uuid(), tenant.id, id, s.sku, s.qty||1).run();
      }
    }
    inserted++;
  }
...
async function updateOrder(request, tenant, orderId, env) {
  const b = await request.json().catch(() => ({}));
  const allowed = ['status','assigned_to','packed_by','completed_at','shelf'];
  const fields = [], values = [];
  for (const key of allowed) {
...
async function deleteOrder(tenant, orderId, env) {
  ...
  ).bind(orderId, tenant.id).run();
  return json({ ok: true });
}

async function clearOrders(tenant, env) {
  await env.DB.prepare(`DELETE FROM order_skus WHERE tenant_id = ?`).bind(tenant.id).run();
  await env.DB.prepare(`DELETE FROM orders WHERE tenant_id = ?`).bind(tenant.id).run();
  return json({ ok: true });
}
...
async function createTrip(request, tenant, env) {
  ...
  await env.DB.prepare(
    ...
    b.driver||'', b.destination||'',
    b.depart_time||null, b.return_time||null,
    b.odo_start||null, b.odo_end||null,
    b.miles||null, b.notes||null,
...
async function createReturn(request, tenant, env) {
  ...
  await env.DB.prepare(
    ...
    b.tracking||null, b.carrier||null,
    b.type||null, b.condition||null,
    b.sku||null, b.qty||1,
    b.pile||null, b.location||null,
    b.notes||null, b.scanned_by||null,
    b.mgr_review?1:0,
...
async function deleteReturn(tenant, returnId, env) {
  await env.DB.prepare(
    `DELETE FROM returns WHERE id = ? AND tenant_id = ?`
  ).bind(returnId, tenant.id).run();
  return json({ ok: true });
}
...
async function getFBA(tenant, env) {
  ...
  return json({ shipments: shipments.map(s => ({ ...s, skus: skuMap[s.id]||[] })) });
}
...
async function createFBA(request, tenant, env) {
  ...
  await env.DB.prepare(
    ...
    b.shipment_id||'', b.fulfillment_center||null,
    b.units||null, b.boxes||null,
    b.dims||null, b.weight||null,
    b.carrier||null, b.tracking||null,
    b.submitted_by||null, b.notes||null,
    ...
    for (const s of b.skus) {
      await env.DB.prepare(
        `INSERT INTO fba_skus (id, tenant_id, shipment_id, sku, qty)
         VALUES (?, ?, ?, ?, ?)`
      ).bind(uuid(), tenant.id, id, s.sku, s.qty||0).run();
    }
  }
  return json({ id }, 201);
}
...
async function saveSettings(request, tenant, env) {
  ...
  for (const [key, value] of Object.entries(body)) {
    ...
    ).bind(tenant.id, key, JSON.stringify(value)).run();
  }
  return json({ ok: true });
}
    ...
    plans: [
      {
        ...
        features:    ['Order queue & scanning','PDF label import','Packer time tracking',
                      'Vehicle log','Returns intake','FBA outbound','Excel exports'],
      },
      {
        id:          'pro',
        name:        'Warehub Pro',
        ...
        features:    ['Everything in Starter','Unlimited packers','Multi-location support',
                      'FedEx live tracking','Priority support','Early access to new features'],
      },
    ]
  });
}
...
async function createCheckoutSession(request, tenant, auth, env) {
  ...
  if (!customerId) {
    const customer = await stripeRequest('/v1/customers', 'POST', {
  ...
  if (session.error) return err('Stripe session error: ' + session.error.message);
...
async function handleStripeWebhook(request, env) {
  ...
  try {
    ...
    if (event.type === 'customer.subscription.updated' ||
      ...
      const plan    = priceId === STRIPE_PRICES.pro ? 'pro' :
                      priceId === STRIPE_PRICES.starter ? 'starter' : 'trial';
      const active  = status === 'active' || status === 'trialing';
      const expiry  = active ? null :
        new Date(sub.current_period_end * 1000).toISOString();
      await env.DB.prepare(
        `UPDATE tenants SET plan = ?, plan_expires_at = ?, stripe_subscription_id = ?
         WHERE stripe_customer_id = ?`
      ).bind(active ? plan : 'trial', expiry, sub.id, custId).run();
    }
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (tenantId && session.customer) {
        await env.DB.prepare(
          `UPDATE tenants SET stripe_customer_id = ? WHERE id = ?`
        ).bind(session.customer, tenantId).run();
      }
    }
    ...
    await env.DB.prepare(
      `INSERT INTO billing_events (id, stripe_event, event_type)
       VALUES (?, ?, ?)`
    ).bind(uuid(), body.slice(0, 4000), event.type || 'unknown').run();

  } catch(e) { console.error('[Warehub webhook]', e.message); }
...
async function logEvent(request, tenant, auth, env) {
  ...
  await env.DB.prepare(
    ...
    uuid(), tenant.id, auth.userId,
    eventType, entityType || null, entityId || null,
...
async function analyzeTenant(tenant, env) {
  ...
  const { results: eventCounts } = await env.DB.prepare(
    `SELECT event_type, COUNT(*) as cnt,
     ...
     GROUP BY event_type`
  ).bind(tenant.id, thirtyDaysAgo).all();
  ...
  const { results: dailyActivity } = await env.DB.prepare(
    `SELECT date(occurred_at, 'unixepoch') as day,
     ...
     ORDER BY day DESC`
  ).bind(tenant.id, sevenDaysAgo).all();
  ...
  console.log(`[Warehub AI] ${aiInsights.length} insights written for tenant ${tenant.id}`);
}
...
function generateRuleBasedInsights(ctx) {
  ...
  if (packed > 0) {
    ...
    insights.push({
      ...
      body:         `Your warehouse processed ${packed} orders in the last 30 days, averaging ${perDay}/day. ` +
  ...
  if (packed > 0 && returns > 0) {
    ...
    insights.push({
      ...
      body:         `${returns} returns logged against ${packed} packed orders this month (${rate}% rate). ` +
                    (sev === 'warning' ? 'This is above typical 10% — investigate which SKUs are driving returns.' :
  ...
  if (fba > 0) {
    insights.push({
      ...
      body:         `On average you're sending ${(fba / 4).toFixed(1)} FBA batches per week. ` +
  ...
  if (!ctx.yesterday.length) {
    insights.push({
      ...
      body:         'The warehouse was quiet yesterday — or data was not synced. ' +
